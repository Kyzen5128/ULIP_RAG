    ...
    def forward(self, data_dict):
        ...
        def check_tensor(tensor, name):
            ...
            if has_nan or has_inf:
                print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                print(f"!!! DIAGNOSTIC ERROR: Invalid values detected in '{name}' output.")
                print(f"!!! Has NaN: {has_nan}, Has Inf: {has_inf}")
                print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                raise ValueError(f"Invalid values in {name}")
            else:
                print(f"--- DIAGNOSTIC INFO: '{name}' output is clean (no NaN/Inf). ---")
        # --- 結束診斷 ---

        text_features = self.text_encoder(data_dict["tokens"], data_dict)
        #check_tensor(text_features, 'Text Encoder')
        ...
        if self.image_encoder is not None:
            image_features = self.image_encoder(data_dict["images"].flatten(end_dim=1), data_dict)
            #check_tensor(image_features, 'Image Encoder')
            output_dict["image_features"] = image_features

        if self.voxel_encoder is not None:
            voxel_features = self.voxel_encoder(data_dict["voxels"], len(data_dict["model_id"]))
            #check_tensor(voxel_features, 'Voxel Encoder')
