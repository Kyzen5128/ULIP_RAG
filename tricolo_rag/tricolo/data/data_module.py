...
def _collate_fn(batch):
    if batch[0]["clip_embeddings_img"] is not None:
        default_collate_items = ("model_id", "category", "tokens", "images", "clip_embeddings_img", "clip_embeddings_text", "text")
    else:
        default_collate_items = ("model_id", "category", "tokens", "images", "text")
    ...
    for i, b in enumerate(batch):
        ...
        voxels_locs_stacked.append(
            torch.cat((torch.full(size=(b["locs"].shape[0], 1), fill_value=i, dtype=torch.int), b["locs"]), dim=1)
        )
