...
# 舊版 RAG dataset。現役 rag_collate_fn 在 core/data/dataset_3d.py:856。
# 僅供歷史參考,勿修改勿引用。
# =====================================================================
'''
...
# ... (ModelNet Class remains unchanged, so it is omitted here for brevity) ...
...
class ShapeNet(data.Dataset):
    def __init__(self, config):
        ...
        # =====================================================================
        # ============== START: RAG Adapter Integration Modification ========
        ...
        # ============== END: RAG Adapter Integration Modification ==========
        # =====================================================================
    ...
    def __getitem__(self, idx):
        ...
        if self.use_caption_templates:
            # 雖然原始設定為 False，但保留邏輯以防萬一
            for template in self.templates:
                raw_caption = template.format(base_caption)
                tokenized_captions_list.append(self.tokenizer(raw_caption))
        else:
            raw_caption = base_caption
            tokenized_captions_list.append(self.tokenizer(raw_caption))

        # 原始 ULIP 會堆疊多個模板的結果，這裡我們只用一個，所以用 [0]
        # 如果您需要多個模板，請修改此處邏輯
        tokenized_captions = tokenized_captions_list[0]

        if self.use_rag_adapter:
            # 如果啟用 RAG，返回元組 (tokens, raw_string)
            text_output = (tokenized_captions, raw_caption)
        else:
            # 否則，保持原始行為
            text_output = tokenized_captions
        # ============== END: RAG Adapter Integration Modification ==========
        # =====================================================================
        ...
        try:
            ...
            image = None # 修改: 如果圖片損壞，返回 None，由 collate_fn 過濾

        return sample['taxonomy_id'], sample['model_id'], text_output, data, image

    def __len__(self):
        return len(self.file_list)

# ... (Objaverse_Lvis_Colored Class remains unchanged) ...
@DATASETS.register_module()
...
# ---------------------------------------------------------------------
# 修改過的 Collate Function 以過濾損壞的數據
# ---------------------------------------------------------------------
...
def customized_collate_fn(batch):
    ...
    if len(batch) == 0:
        # 如果整個批次都被過濾掉了，返回 None，讓訓練循環可以跳過
        print_log(f"Warning: A whole batch of {original_len} samples was corrupted and filtered out.", "DataLoader")
        return None
...
class Dataset_3D():
    ...
    def build_3d_dataset(self, args, config):
        ...
        # 將 use_rag_adapter 參數傳遞到最終的數據集配置中
        config.use_rag_adapter = getattr(args, 'use_rag_adapter', False)
