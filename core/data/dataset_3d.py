...
class Dataset_3D():
    def __init__(self, args, tokenizer, dataset_type, train_transform=None):
        ...
        with open(os.path.join(_CORE_DIR, 'data/configs/dataset_catalog.json'), 'r') as f:
            ...
            entry = self.dataset_catalog[self.dataset_name]
            self.dataset_usage = entry['usage']
            # Respect an explicit train/val split when the catalog provides it.
            # The previous usage-only lookup made IKEA validation silently reuse
            # the training split because usage="train" for both loaders.
            self.dataset_split = entry.get(dataset_type, entry[self.dataset_usage])
            self.dataset_config_dir = entry['config']
