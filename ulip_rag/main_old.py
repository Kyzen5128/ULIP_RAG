...
def get_args_parser():
    ...
    parser.add_argument('--use_rag_adapter', action='store_true', help='Use RAG Adapter model.')
    ...
    parser.add_argument('--freeze_main_model', action='store_true', help='Freeze main model and only train the RAG adapter.')
...
def main(args):
    ...
    if args.use_rag_adapter:
        print("=> creating model with RAG Adapter: ULIP_PointBERT_RAG")
        model = models.ULIP_PointBERT_RAG_GENERATIVE(args) 
    else:
        print("=> creating model: {}".format(args.model))
        model = getattr(models, args.model)(args=args)
    ...
    if args.use_rag_adapter:
        criterion = models.ULIP_Loss_Generative_RAG(args).cuda(args.gpu)
...
#         # 根據返回的元素數量解包
#         #pc, text, image = inputs[3], inputs[2], inputs[4]
#         pc, text, image = inputs
#         # 將需要計算的 Tensors 移動到 GPU
...
#             # 這是 RAG 模式，text 是一個元組 (tokens, raw_strings)
#             # 只將 tokens tensor 移動到 GPU
#             tokenized_text = text[0].cuda(args.gpu, non_blocking=True)
#             raw_text = text[1]
#             # 將 text 重新打包成模型 forward 函數期望的格式
#             text_input_to_model = (tokenized_text, raw_text)
#         else:
#             # 這是標準模式，text 本身就是一個 tensor
#             text_input_to_model = text.cuda(args.gpu, non_blocking=True)
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, args):
    ...
    metric_names_from_model = models.get_metric_names(args.model)
    if args.training_strategy == 'staged_1':
        metric_names = ['loss', 'loss_2d_enhanced', 'loss_consistency']
    else: # staged_2 or joint
        metric_names = ['loss', 'loss_3d_enhanced', 'loss_2d_enhanced']
    metric_names = list(OrderedDict.fromkeys(metric_names + metric_names_from_model))

    iters_per_epoch = len(train_loader) // args.update_freq
    metrics = OrderedDict([(name, AverageMeter(name, ':.4e')) for name in metric_names])
    progress = ProgressMeter(
        len(train_loader), # 改為使用 train_loader 的總長度
        [batch_time, data_time, mem, *metrics.values()],
        prefix="Epoch: [{}]".format(epoch))
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        # --- 資料解包與傳輸 ---
        if args.use_rag_adapter:
            pc, text_input_from_loader, image = batch_data
            tokenized_text = text_input_from_loader[0].cuda(args.gpu, non_blocking=True)
            raw_text = text_input_from_loader[1]
            text_input_to_model = (tokenized_text, raw_text)
        else:
        ...
        for k in loss_dict:
            if k in metrics:
                # 直接使用當前 GPU 的損失值進行更新
                # 分散式同步將在 epoch 結束時由 progress.synchronize() 統一處理
                metrics[k].update(loss_dict[k].item(), pc.size(0))

        batch_time.update(time.time() - end)
        end = time.time()
        mem.update(torch.cuda.max_memory_allocated() / 1e9)

        # +++ 關鍵修正：將 progress.display 放在 print_freq 條件內 +++
